...
###############################################################################
import alignment_classes as cl
import datetime
...
# order reads by their location in the genome and an ordered lsit of read names
def order_reads_by_mapped_idx(reads: list["cl.Read"]) -> list[str]:
    ordered_reads = sorted(reads, key=lambda x: x.mapped_idx if x.mapped_idx is not None else float('inf'))
    return [read.label for read in ordered_reads if read.mapped_idx is not None]
